...
class AuthLoggedInScope extends Scope<BaseAuthScopeArgs> {
  static const String scopeName = 'auth-logged-in';

  AuthLoggedInScope()
    : super(name: "auth-logged-in", args: BaseAuthScopeArgs());
}
...
class AuthScope {
  ...
  static AuthLoggedInScope loggedIn() => AuthLoggedInScope();

  static AuthLoggedOutScope loggedOut() => AuthLoggedOutScope();

  static String get loggedInScopeName => "auth-logged-in";
...
class AuthScopeHandler extends SwitchScopeHandler<BaseAuthScopeArgs> {
  ...
  final _allScopeNames = ['auth-logged-in', 'auth-logged-out'];
  ...
  Future<void> onEnterScopeByScope(Scope<dynamic> scope) async {
    if (scope.name == 'auth-logged-in') {
      await _scopeHandlerDelegate._loggedIn(weaverInstance);
    }

    if (scope.name == 'auth-logged-out') {
      await _scopeHandlerDelegate._loggedOut(weaverInstance);
    }
  }
...
class AuthScopeOnWeaver {
  ...
  bool get isLoggedIn => weaverInstance.isInScope("auth-logged-in");
